...
void VariantVisitor::visit(trace::Enum *e)
{
    ...
    sig = ApiTrace::getInstance()->enumSignature(e->sig->id);
    if (!sig) {
        sig = new ApiTraceEnumSignature(e->sig);
        ApiTrace::getInstance()->addEnumSignature(e->sig->id, sig);
    }
      ...
      m_parentFrame(parentFrame)
{
    ...
    m_signature = ApiTrace::getInstance()->signature(call->sig->id);
    ...
    if (!m_signature) {
        ...
        ApiTrace::getInstance()->addSignature(call->sig->id, m_signature);
    }
    if (call->ret) {
